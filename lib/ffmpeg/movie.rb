...
module FFMPEG
  class Movie
    ...
    def initialize(path, options = {})
      ...
      if remote?
        unless options[:skip_resource_check]
          @head = head
          unless @head.is_a?(Net::HTTPSuccess)
            raise Errno::ENOENT, "the URL '#{path}' does not exist or is not available (response code: #{@head.code} #{@head.msg})"
          end
        end
      else
